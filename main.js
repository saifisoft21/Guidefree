    ...
    ps.forEach(p => {
        ...
        c.innerHTML = `
            ...
            <div style="padding:1.2rem;flex:1;display:flex;flex-direction:column;">
                <h3 style="font-size:1.1rem;font-weight:900;margin-bottom:0.5rem;">${_e(p.title)}</h3>
                <p style="color:var(--text-secondary);font-size:0.85rem;margin-bottom:1.5rem;line-height:1.4;flex:1;">${_e(p.content)}</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (s) s.oninput = (e) => { _sq = e.target.value.trim(); _fr(); };
